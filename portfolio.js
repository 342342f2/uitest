...
const ipgeolocation = 'https://api.ipgeolocation.io/ipgeo?apiKey=f69125b137074f4fa19a603d19b34d47';
...
$(document).ready(() => {
  const links = [
    ...
    {
      name: 'russianblackhat',
      link: 'russianblackhats',
    },
    {
      name: '911',
      link: 'spademp',
  ...
  for (let i in links) {
    ...
    if (i != links.length - 1) $('#marquee').append(' <img class="emoticon" src="assets/others/akm.gif"> ');
  }
  ...
  app.titleChanger(['corona', 'Team 911', 'would you', 'like to', 'be doxxed ?!', '=', 'corona.exe', 'Song name: KUSO GVKI - Cinnamon (feat. Apfel) (splash! Mag Premiere)']);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      ...
      writeLine([`<i style='color: #F62459'>$$$ corona.exe $$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
